...
const BACKLOG = [
    { id: 1, title: 'CVSS score for KEV feed rows', status: 'planned',
      desc: 'The CISA KEV catalog carries no CVSS score, so KEV-feed rows show a blank CVSS. Enrich KEV rows with CVSS (from NVD / CVEfeed.io).' },
    { id: 2, title: 'Column: Remote-code-exploit availability', status: 'planned',
      desc: 'Flag CVEs that have a known remote code execution exploit, derived from exploit intel and weakness type.' },
    { id: 3, title: 'Column: Exploit availability', status: 'planned',
      desc: 'A clear at-a-glance column for whether any public exploit/PoC exists for a CVE.' },
    ...
    { id: 5, title: 'Light theme', status: 'planned',
      desc: 'A light/dark theme toggle, persisted per browser, across all pages.' },
    { id: 6, title: 'CVEfeed.io integration', status: 'planned', blocked: true,
      desc: 'Use the CVEfeed.io API for richer CVE details. Blocked on the API key + endpoint documentation.' },
    { id: 7, title: 'CVSS-vector search', status: 'planned',
      desc: 'A search box to query CVEs by CVSS vector components (e.g. AV:N / AC:L / PR:N), backed by the NVD cvssV3Metrics filter.' },
    { id: 8, title: '.env for API keys (NVD + CVEfeed.io)', status: 'planned',
      desc: 'Load NVD and CVEfeed.io keys from a .env file for better data availability and higher rate limits.' },
